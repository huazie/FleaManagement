...
Huazie.ajax = {
	 ...
	 *            cmd json对象参数
	 * @param {Function}
	 *            callback(data, status) GET请求成功回调函数
	 */
	getJson		: function(url, cmd, callback){
		if(arguments.length == 2){
			callback = cmd, cmd = '';//设置请求参数为空
		}
		var reqMode = this.reqMode.GET;
		var dataType = this.dataType.TEXT;
		this.ajax(url, reqMode, cmd, dataType, callback);
	},
	/**
	...
	getJsonSync		: function(url, cmd, callback){
		...
		var reqMode = this.reqMode.GET;
		var dataType = this.dataType.TEXT;
		this.ajax(url, reqMode, cmd, dataType, callback, true);
	},
	/**
	 ...
	 *            cmd json对象参数
	 * @param {Function}
	 *            callback(data, status)  POST请求成功回调函数
	 */
	 ...
	 * @method ajax
	 * @param {String}
	 *            url HTTP(POST/GET) Request Path
	 * @param {String}
	 *            reqMode POST/GET
	 * @param {Object}
	 *            cmd json data
	 ...
	 *            callback  请求成功回调函数
	 * @param {String}
	 * 			  sysc (true: 异步,false:同步)
	 */
	ajax 	 	: function(url, reqMode, cmd, dataType, callback, sync) {
		var _data  = cmd;
		var _self  = Huazie.ajax;
		var _async = sync ? false : true;
		var _cache = (dataType == _self.dataType.HTML) ? true : false;//如果是html数据类型，就采用缓存方式加载
		$.ajax({
			url	 	 	: 	url,
			type 	 	: 	reqMode,
			data 	 	: 	_data,
...
Huazie.tpl = {
	 ...
	 * @method loadTpl
	 * @param {String}
	 * 				tplPath (Path of outer tpl file)
	 * @param {Function}
	 * 				callback 回调函数
	 */
...
Huazie.browser = {
	 ...
	 * @returns
	 */
	getFrameParamter : function(name){
...
Huazie.dialog = {
	 ...
	 * 右击弹出气泡浮层
	 */
	rightClickBuddle 		: function(data, obj, callback){
		Huazie.tpl.loadTpl(TplUrlMap.get("dialog"), function(){
			var d = dialog({
				padding : 2,
