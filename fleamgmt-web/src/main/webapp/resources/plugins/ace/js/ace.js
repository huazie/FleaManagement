...
ace.handle_side_menu = function(a) {
	...
	a(".nav-list").on(ace.click_event, function(h) {
		...
		if (!g.hasClass("dropdown-toggle")) {
			...
			if (ace.click_event == "click" && !a(g).parent().hasClass('active')) {
				if(a(g).parent().parent().hasClass('nav-list')){
					$('#nav-list').find("> .active > .submenu").slideUp(200);
					a(g).parent().siblings().removeClass('active open');
				}
				...
				a(g).parent().siblings().find(".submenu").slideUp(200);
				a(g).parent().addClass('active').siblings().removeClass('active open');
			}
			return
		}
		var f = g.next().get(0);
		if (!a(f).is(":visible")) {
			...
			if (ace.click_event == "click" && !a(g).parent().hasClass('active')) {
				...
				a(g).parent().find(".active").removeClass('active open');
				a(g).parent().siblings().find(".submenu").slideUp(200);
				a(g).parent().addClass('active').siblings().removeClass('active open');
				a(f).hide();
				if(c && d.hasClass('nav-list')){
					a(g).parent().siblings().find("> .submenu").hide();
				}
			}
		} else {
	...
	a("#skin-colorpicker").on(
			...
			function() {
				...
				if (d == "skin-2") {
					...
					a(".ace-nav > li:not(:last-child)").addClass("light-pink").find('> a > [class*="fa fa-"]').addClass(
							"pink").end().eq(0).find(".badge").addClass("badge-warning")
				} else {
					a(".ace-nav > li").removeClass("no-border margin-1");
					a(".ace-nav > li:not(:last-child)").removeClass("light-pink").find('> a > [class*="fa fa-"]')
							.removeClass("pink").end().eq(0).find(".badge").removeClass("badge-warning")
				}
				if (d == "skin-3") {
...
ace.switch_direction = function(d) {
	...
	c.toggleClass("rtl").find(".dropdown-menu:not(.datepicker-dropdown,.colorpicker)").toggleClass("pull-right").end()
			.find(".pull-right:not(.dropdown-menu,blockquote,.profile-skills .pull-right)").removeClass("pull-right")
			.addClass("tmp-rtl-pull-right").end().find(".pull-left:not(.dropdown-submenu,.profile-skills .pull-left)")
			.removeClass("pull-left").addClass("pull-right").end().find(".tmp-rtl-pull-right").removeClass(
					"tmp-rtl-pull-right").addClass("pull-left").end().find(".chosen-container").toggleClass(
					"chosen-rtl").end();
	function a(h, g) {
		c.find("." + h).removeClass(h).addClass("tmp-rtl-" + h).end().find("." + g).removeClass(g).addClass(h).end()
